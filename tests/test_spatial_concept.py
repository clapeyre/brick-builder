...
class TestSpatialConceptSession:
    def test_success_preserves_request_and_returns_stable_bounded_concepts(self):
        ...
        assert result["concepts"][0]["geometry"][0]["ref"] == "concept-1/box-1"
        assert result["concepts"][0]["render"]["geometry_refs"] == ["concept-1/box-1"]
    ...
    def test_clarification_is_a_terminal_actionable_result(self):
        ...
        assert result["clarification"] == "Should it be a vehicle or a creature?"
        with pytest.raises(SpatialConceptError):
    ...
    def test_concept_ids_cannot_escape_render_artifact_root(self):
        ...
        result = session.submit(bad)
        assert result["status"] == "pending"
        assert "concept 1.id" in result["feedback"][0]

    def test_serialization_is_deterministic(self):
        session = SpatialConceptSession("A boxy boat")
        session.submit(response())
        encoded = session.serialize()
        assert encoded == session.serialize()
        assert json.loads(encoded) == session.snapshot()
    ...
    def test_success_writes_reproducible_previews_and_session_artifact(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            assert (Path(directory) / "one" / "render-concept-1.svg").read_bytes() == (Path(directory) / "two" / "render-concept-1.svg").read_bytes()
            assert first["request_text"] == "Make a boxy boat"
            assert first["renders"][0]["sha256"]
